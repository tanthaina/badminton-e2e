...
4. **ประวัติ คลังสต็อก และอื่นๆ:**
   - **(ใหม่ล่าสุด)** ระบบคลังสต็อกลูกแบด (Inventory System) พร้อมตั้งค่าเพิ่ม/ลดสต็อกเป็นหลอด ตัดลูกแบดอัตโนมัติเมื่อบันทึกเกม คืนสต็อกเมื่อลบเกม และแจ้งเตือนเมื่อลูกแบดใกล้หมด
...
- **(ล่าสุด) เพิ่มระบบคลังสต็อกลูกแบด (Inventory System)**
  - เพิ่มระบบตั้งค่าคลังสต็อก สามารถเพิ่ม/ลดสต็อกเป็นหลอด (12 ลูก / 60 ลูก) หรือทีละลูกได้
  - ตัดสต็อกอัตโนมัติเมื่อมีการบันทึกเกมในหน้ารายวัน และคืนสต็อกอัตโนมัติหากมีการลบเกม
  - แจ้งเตือนบนหน้าจอ (Badge) หากลูกแบดในสต็อกเหลือน้อยกว่า 1 หลอด (< 12 ลูก)
  - เพิ่มชุดทดสอบ Cypress `19-inventory-system.cy.js` สำหรับทดสอบระบบสต็อก
- **(ล่าสุด ✅ 2026-07-16) แก้ไขบั๊กการติดตั้ง PWA บน Android (PWA Install Prompt Fix)**
  - **ปัญหา:** ผู้ใช้ Android กดเพิ่มลงหน้าจอหลักแล้วได้เป็นเพียง Bookmark (ไอคอนรูป G) แทนที่จะเป็นไอคอนของแอป
  - **สาเหตุ & วิธีแก้:** พบว่าชื่อไฟล์รูปภาพในระบบเป็น `icon.png.png` แต่ในโค้ดเรียกหา `icon.png` ทำให้โหลดรูปไม่ขึ้นและไม่ผ่านเกณฑ์ PWA จึงได้ทำการแก้ไขพาธรูปภาพใน `manifest.json`, `index.html`, และ `sw.js` ให้ตรงกัน พร้อมอัปเดตเวอร์ชัน Service Worker แคชเป็น `v8` เพื่อบังคับให้โหลดการตั้งค่าใหม่

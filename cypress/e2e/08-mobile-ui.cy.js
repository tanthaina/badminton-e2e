describe('08 - Mobile UI & Responsive Design', () => {
  ...
  it('ทดสอบ 3: ชื่อผู้เล่นยาวๆ ในหน้าบัญชีต้องไม่เบียดปุ่มทวง/จ่ายตกขอบ (Break Word)', () => {
    ...
    // ตรวจสอบว่ากล่องค้างชำระใช้ flex-col (สำหรับมือถือ) และชื่อถูกสั่งให้ตัดคำแบบสวยงาม (break-words)
    cy.contains('#unpaid-list-overall > div', longName).should('have.class', 'flex-col');
    cy.contains('#unpaid-list-overall > div', longName).find('div.break-words').should('be.visible');
    
    // ปุ่ม 'ทวงส่วนตัว (LINE)' และ 'จ่าย' ต้องยังอยู่ในหน้าจอ
    cy.contains('#unpaid-list-overall > div', longName).find('button').contains('จ่าย').should('be.visible');
  });

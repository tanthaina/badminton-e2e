describe('12 - PWA Offline Support (Service Worker & Cache)', () => {
  ...
  it('ทดสอบ 2: จำลองการตัดอินเทอร์เน็ต (Offline Mode) แอปต้องยังใช้งานได้', () => {
    ...
    // รอจนกว่า Cache จะถูกสร้างขึ้นมาจริงๆ แทนการใช้เวลาตายตัว (เช็คแบบยืดหยุ่นรองรับทุกเวอร์ชัน v)
    cy.window().then(win => win.caches.keys().then(keys => keys.some(k => k.startsWith('badminton-pay-v')))).should('be.true');
